...
from utils import build_prompt, baichuan_2_response
        ...
        for line in f:
            ...
            if "term" in line and line["term"] == word:
                for p in range(3):
                    ...
                    lsb['response'] = baichuan_2_response(prompt, model, tokenizer)
...
def post_test_open_source(pre_prompt, word, task, model, tokenizer):  
    with open(f'benchmark/{task}_clean.json', 'r', encoding='utf-8') as f,\
        open(f'test/results/post_{task}_baichuan_BASE.json', 'a', encoding='utf-8') as w:
        for line in f:
            line = json.loads(line.strip())
            if "term" in line and line["term"] == word:
                for p in range(3):
