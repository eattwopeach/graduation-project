...
with open('benchmark/new_term.txt', 'r', encoding='utf-8') as f1, open('benchmark/target_new.txt', 'r', encoding='utf-8') as f2:
    ...
    for _ in range(2):
        ...
        old_post_qwen_results = filtered_outputs
        ...
        embeddings_post_qwen_old = mean_pooling(model_output_post_qwen_old, encoded_input_post_qwen_old['attention_mask'])
        embeddings_pre_qwen_old = mean_pooling(model_output_pre_qwen_old, encoded_input_pre_qwen_old['attention_mask'])
        similarities_old = []
        for i in range(100):
            # 计算余弦相似度
