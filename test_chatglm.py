...
from utils import build_prompt, chatglm_2_response
...
def test_open_source(word, task, model, tokenizer):  
    with open(f'benchmark/{task}_clean.json', 'r', encoding='utf-8') as f,\
        open(f'test/results/{task}_chatglm_BASE.json', 'a', encoding='utf-8') as w:
        for line in f:
            ...
            if "term" in line and line["term"] == word:
                for p in range(3):
                    ...
                    lsb['response'] = chatglm_2_response(prompt, model, tokenizer)
...
def post_test_open_source(pre_prompt, word, task, model, tokenizer):  
    with open(f'benchmark/{task}_clean.json', 'r', encoding='utf-8') as f,\
        open(f'test/results/post_{task}_chatglm_BASE.json', 'a', encoding='utf-8') as w:
        for line in f:
            line = json.loads(line.strip())
            if "term" in line and line["term"] == word:
                for p in range(3):
